// See https://github.com/karan101292/MathquillBasedEditor
function MathEditor(id) {
    ...
    // new MqButton( "\\ge", "≥" ),
    // new MqButton( "\\in" ),
    // new MqButton( "X_{i}", "_{}" ).addHint( "use '_' as keyboard shortcut" ),
    //
    // new MqButton(),
    // new MqButton( "\\frac{a}{b}", "\\frac{}{}" ).addHint( "use '/' as keyboard shortcut" ),
    // new MqButton( "x^n", "{}^{}" ).addHint( "use '^' as keyboard shortcut" ),
    // new MqButton( "\\sqrt{x}", "\\sqrt{}" ),
    // new MqButton( "\\sqrt[n]{x}", "\\sqrt[{}]{}" ),
    //
    // new MqButton( "\\overrightarrow{AB}", "\\overrightarrow{}" ),
    //
    // new MqButton(),
    // new MqButton( "[\\:,\\:]", "[{},{}]" ),
    // new MqButton( "[\\:,\\:)", "[{},{})" ),
    ...
    this.template = 'default';
        this.default_toolbar_buttons = ["division", "absvalue", 'multiplication', 'degree', "emptyset", "reals",
            "pi", "euler", "sigma", "infty", "less_equal", "not_equal", "greater_equal", "in",
            "fraction", "power",
             "square_root", "root", 'plus_minus',
            "arrow",
            'closed_interval', 'open_right_interval', 'open_left_interval', 'open_interval'];
    this.default_toolbar_tabs = ["General", "Symbols", "Geometry"];
    button_meta = {
        "emptyset": {latex: "\\emptyset", moveto: "Up", movefor: 1, tab: 1, icon: '\\emptyset'},
        "pi": {latex: "\\pi", moveto: "Up", movefor: 1, tab: 1, icon: '\\pi'},
        "euler": {latex: "e", moveto: "Up", movefor: 1, tab: 1, icon: 'e'},
        "sigma": {latex: "\\sigma", moveto: "Up", movefor: 1, tab: 1, icon: '\\sigma'},
        "infty": {latex: "\\infty", moveto: "Up", movefor: 1, tab: 1, icon: '\\infty'},
        "reals": {latex: "\\R", moveto: "Up", movefor: 1, tab: 1, icon: '\\R'},
        "in" : {latex: "\\in", moveto: "Up", movefor: 1, tab: 1, icon: '\\in'},
        "absvalue": {latex: "\\mid", moveto: "Up", movefor: 1, tab: 1, icon: '\\mid'},
        "power": {latex: "{}_{}", moveto: "Left", movefor: 1, tab: 1, icon: 'x^n'},
        "fraction": {latex: "\\frac{}{}", moveto: "Up", movefor: 1, tab: 1, icon: '\\frac{a}{b}'},
        "mix_fraction": {latex: "\\frac{}{}", moveto: "Up", movefor: 1, tab: 1, icon: '\\frac{\\square}{\\square}'},
        "square_root": {latex: "\\sqrt{}", moveto: "Left", movefor: 1, tab: 1, icon: '\\sqrt{x}'},
        "cube_root": {latex: "\\sqrt[3]{}", moveto: "Left", movefor: 1, tab: 1, icon: '\\sqrt[3]{x}'},
        "root": {latex: "\\sqrt[{}]{}", moveto: "Left", movefor: 2, tab: 1, icon: '\\sqrt[n]{x}'},
        "superscript": {latex: "\\^{}", moveto: "Up", movefor: 1, tab: 1, icon: '\\square^2'},
        "subscript": {latex: "\\_{}", moveto: "Down", movefor: 1, tab: 1, icon: '\\square_{2}'},
        "multiplication": {latex: "\\cdot", tab: 1, icon: '\\cdot'},
        "arrow": {latex: "\\overrightarrow{}", moveto: "Left", movefor: 1, tab: 1, icon: "\\overrightarrow{AB}"},
    ...
    keyboard_keys = {
        'letters': [{'value': 'q', 'type': 'write', 'class': 'ks', 'display': 'q', 'new_line': false}, {
            'value': 'w',
            'type': 'write',
            'class': 'ks',
            'display': 'w',
...
MathEditor.prototype.setTemplate = function (name) {
    if (name == 'floating-toolbar') {
        ...
        setCloseKey(this.topElements, this.answerSpan);
        ...
        jq(this.answerSpan).focusin(function (o) {
            jq(o.currentTarget).css('background-color', '#ffffcc');
            jq.each(mathed_tmp, function (k, v) {
                v.toolbar.hide();
            });
            var id = $(o.currentTarget).attr('id');
        });
        jq(this.answerSpan).focusout(function (o) {
            jq(o.currentTarget).css('background-color', 'white');
        });
    } else {
...
setCloseKey = function(top_elements, answer_span) {
    answer_span.onkeydown = function(evt) {
        evt = evt || window.event;
        if ((("key" in evt) && (evt.key === "Escape" || evt.key === "Esc")) || (evt.keyCode === 27)) {
            if(top_elements.toolbar.is(":visible")) {
                top_elements.toolbar.hide();
            } else {
                top_elements.toolbar.show();
            }
        } else if (event.ctrlKey && event.altKey && ((("key" in evt) && (evt.key === "-")) || (evt.keyCode === 189))) {
            if(!top_elements.toolbar.is(":visible")) {
                removeLastEquation();
            }
        } else if (event.ctrlKey && event.altKey && ((("key" in evt) && (evt.key === "+")) || (evt.keyCode === 187))) {
            if(!top_elements.toolbar.is(":visible")) {
                addEquation();
            }
        } else if (event.ctrlKey && event.altKey && ((("key" in evt) && (evt.key === "S")) || (evt.keyCode === 83))) {
            screenshot();
        }
    };
}
